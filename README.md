...
Example track files can include:
`acc_75m_straight_width3m.csv` for a 75 m acceleration straight,
`skidpad_entry_2right_2left_exit_width3m.csv` for skidpad with entry, two right laps, two left laps and exit,
`autocross_random_track_8_width3m.csv` converted from a centerline-only CSV by assuming a 3 m track width.
---
...
The steering actuator is modeled as a second-order system:
$$\dot{\delta}{\mathrm{cmd}}=u{\delta}$$
$$\dot{\delta}=\dot{\delta}$$
$$\ddot{\delta}=\omega_n^2(\delta_{\mathrm{cmd}}-\delta)-2\zeta\omega_n\dot{\delta}$$
The dynamic bicycle objective function is:
...
`delta_cmd`, `delta`, `delta_dot` steering bounds,
